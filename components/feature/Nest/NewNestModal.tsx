...
import { createNest } from '@/features/nest/actions';
...
export default function NewNestModal({ isOpen, onClose, currentUserId }: NewNestModalProps) {
  ...
  const handleCreate = async () => {
    setError('');
    if (!name.trim()) {
      setError('Nest name is required');
      return;
    }
    ...
    try {
      ...
      if (result.success && result.nestId) {
        ...
        setError(result.error || 'Failed to create Nest');
      }
    } finally {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="New Nest">
      <Input
        label="Nest name"
        value={name}
