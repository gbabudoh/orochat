...
import { createSlate } from '@/features/oroslate/actions';
...
export default function NewSlateModal({ isOpen, onClose, currentUserId, organizationId }: NewSlateModalProps) {
  ...
  const handleCreate = async () => {
    ...
    try {
      const result = await createSlate(organizationId, currentUserId, name, selected);
      if (result.success && result.slateId) {
        onClose();
        router.push(`/oroslate/slate/${result.slateId}`);
      } else {
        setError(result.error || 'Failed to create Slate');
      }
    } finally {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="New Slate">
      <Input
        label="Slate name"
        value={name}
      ...
      {isLoading ? (
        ...
        <p className="text-sm text-gray-500 py-4 text-center">
          You don&apos;t have any connected Oros yet — you can invite people later.
        </p>
      ) : (
