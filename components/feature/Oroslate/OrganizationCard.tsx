...
export default function OrganizationCard({ organization }: { organization: OrgListItem }) {
  ...
  return (
    <Link href={`/oroslate/org/${organization.id}`} className="group block h-full">
      <Card hover className="h-full flex flex-col justify-between overflow-hidden">
        <div>
          {/* Card Banner */}
          <div className="relative overflow-hidden w-full h-24 bg-gradient-to-br from-[#458B9E] via-[#366f7e] to-[#2a5662] rounded-lg mb-4 flex items-center justify-center p-4 shadow-inner">
          ...
          <div className="flex items-start justify-between gap-2 mb-3">
            <h3 className="text-lg font-bold text-gray-900 group-hover:text-[#458B9E] transition-colors truncate">
              {organization.name}
            </h3>
            <span
              className={`shrink-0 text-[11px] font-semibold tracking-wide uppercase px-2.5 py-0.5 rounded-full ${
                isTrialing
        ...
        {/* Footer / Stats */}
        <div className="flex items-center justify-between text-xs font-medium text-gray-600 pt-3 border-t border-gray-100 mt-2">
          <div className="flex items-center gap-2">
            <span className="inline-flex items-center gap-1.5 bg-gray-100/80 px-2 py-0.5 rounded-md ring-1 ring-gray-200/50">
          ...
          <span className="inline-flex items-center gap-1 text-[#458B9E] font-semibold opacity-0 group-hover:opacity-100 transition-opacity">
